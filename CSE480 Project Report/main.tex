...
\begin{itemize}
    \item For the TP2 example instance, the exact minimum number of bins is computed using an exact branch-and-bound procedure, which is feasible due to the small problem size.
    \item For the BinPack benchmark instances, the third value in the instance header provides the known optimal number of bins. We use this as the exact reference.
\end{itemize}
